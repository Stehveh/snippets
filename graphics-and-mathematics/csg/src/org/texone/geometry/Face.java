...
public class Face implements Cloneable {
	...
	public BoundingBox getBound() {
		return new BoundingBox(v1.getPosition(), v2.getPosition(), v3.getPosition());
	}
	...
	public Vertex getNormal() {
		...
		Vertex p1 = v1.getPosition();
		Vertex p2 = v2.getPosition();
		Vertex p3 = v3.getPosition();
		Vertex xy, xz;

		xy = new Vertex(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
		xz = new Vertex(p3.x - p1.x, p3.y - p1.y, p3.z - p1.z);
	...
	public double getArea() {
		...
		Vertex p2 = v2.getPosition();
		Vertex p3 = v3.getPosition();
		Vertex xy = new Vertex(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
		Vertex xz = new Vertex(p3.x - p1.x, p3.y - p1.y, p3.z - p1.z);
	 ...
	 */
	public void rayTraceClassify(Mesh object) {
	 ...
	 */
	private boolean hasPoint(Vertex point) {
	 ...
	 */
	private static int linePositionInX(Vertex point, Vertex pointLine1, Vertex pointLine2) {
		double a, b, z;
		if (
			(Math.abs(pointLine1.y - pointLine2.y) > TOL) && (((point.y >= pointLine1.y) && (point.y <= pointLine2.y)) || ((point.y <= pointLine1.y) && (point.y >= pointLine2.y)))) {
	...
	private static int linePositionInY(Vertex point, Vertex pointLine1, Vertex pointLine2) {
		double a, b, z;
	...
	private static int linePositionInZ(Vertex point, Vertex pointLine1, Vertex pointLine2) {
		double a, b, y;
		if ((Math.abs(pointLine1.x - pointLine2.x) > TOL) && (((point.x >= pointLine1.x) && (point.x <= pointLine2.x)) || ((point.x <= pointLine1.x) && (point.x >= pointLine2.x)))) {
